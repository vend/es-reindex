...
def retried_request method, url, data=nil, headers={content_type: 'application/json'}
  while true
    begin
      ...
      return nil
    rescue RestClient::Conflict # record already exists or is newer than source
      return nil
...
while true do
  ...
  data['hits']['hits'].each do |doc|
    ...
    if single
      version = doc['_source']['_version'] ? doc['_source']['_version'] : 1
      if retried_request :post, "#{durl}/#{didx}/#{doc['_type']}/#{doc['_id']}?version=#{version}&version_type=external_gte", Oj.dump(doc['_source']) + "\n"
        changed += 1
      end
    else
...
printf "#{changed} documents created/updated\n" if changed > 0
